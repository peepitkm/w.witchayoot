var Calculator = {
	...
	results_id:    '#billing-cash-payment',
	results_value: '0',
	memory_id:     '#billing-cash-payment',
	memory_value:  '',
	...
	calculate: function() {
		...
		console.log("Memory Value:" + this.memory_value);
		this.results_value = this.engine.exec(this.memory_value);
		console.log("Result Value:" + this.results_value);
		this.memory_value = this.results_value
		//this.add_to_history();
		this.refresh();
	},
	...
	put: function(value) {
		...
		this.memory_value += value.replace(",", "");
		this.update_memory(); // use for show memory value
	},
	...
	update_result: function() {
		$(this.results_id).val(this.results_value).trigger("change");
		//document.getElementById(this.results_id).innerHTML = this.results_value;
	},

	update_memory: function() {
		$(this.memory_id).val(this.memory_value).trigger("change");
...
$(document).ready(function() {
	$("#calculator-wrapper .btn").click(function(e) {
		e.preventDefault();
